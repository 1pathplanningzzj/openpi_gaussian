...
class VGGT4DGSModel(torch.nn.Module):
    def __init__(self, sh_degree, height, width, min_depth, max_depth):
        ...
        vggt_path = '/workspace/kuntaoxiao/.cache/torch/hub/checkpoints/model.pt'
        ...
        for degree in range(1, self.sh_degree + 1):
            self.sh_mask[degree**2 : (degree + 1) ** 2] = 0.1 * 0.5**degree
        ...
        self.pts_dim = self.gs_dim // 2
        self.pts_proj = nn.Linear(5, self.pts_dim)
        self.dpt_dim = self.depth_dim+self.gs_dim+self.pts_dim
        # self.dpt_proj_morm = nn.LayerNorm(self.dpt_dim)
        self.dpt_feat_proj2d = nn.Sequential(
            nn.Linear(self.dpt_dim, self.dpt_dim*2),
            nn.GELU(),
            nn.Linear(self.dpt_dim*2, self.dpt_dim),
        )
    ...
    def get_all_kernel_maps(self, context_dpt_feat, context_Ks, context_e2c_extrs):
        ...
        depth_conf = rearrange(depth_conf, 'b (f c) h w d-> b f c d h w',c=self.num_cams)
    ...
    def refine_dpt_feat_with_render_infos(self, context_dpt_feat, context_pts, render_Ks, render_e2c_extrs, render_relative_frames):
        ...
        for frame_idx in range(num_render_frames):
            context_pts_emb = self.pts4d_to_emb(context_pts, render_relative_frames[:,frame_idx])
            context_data = torch.cat([context_dpt_feat, context_pts_emb], dim=-1)

            context_data = rearrange(context_data, 'b f c n d -> b (f c n) d')
            # context_data = self.dpt_proj_morm(context_data)
            context_data = context_data + self.dpt_feat_proj2d(context_data)

            # proj_2dcoord: B, num_camera, num_ctx, 2
            # proj_vaildmask: B, num_camera, num_ctx
            proj_2dcoord, proj_validmask = \
                self.project_points_map_to_pixel_coord(context_pts,render_Ks[:,frame_idx],render_e2c_extrs[:,frame_idx])
    ...
    def project_points_map_to_pixel_coord(self, point_map, K, e2c_extr):
        '''
        point_map: Batch_size, frame_ctx, num_camera, num_ctx, 3
        K: B, num_camera, 4, 4 (only use [:, :, :3, :3])
        e2c_extr: B, num_camera, 4, 4

        '''

        point_map = rearrange(point_map, 'b f c p d -> b (f c p) d')
