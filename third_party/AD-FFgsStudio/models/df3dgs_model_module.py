...
class DF3DGS_LITModelModule(pl.LightningModule):
    def __init__(self, cfg, save_dir='.', logger=None):
        super().__init__()
        self.read_config(cfg)

        self.save_dir = save_dir
        self.model = DF3DGSModel(cfg)
        self.model.load_official_weights()
    ...
    def init_novel_view_mode(self, recontrast_ids=[0], render_ids=[0]):
        ...
        self.render_frame_ids = render_ids
    ...
    def aug_novel_view_mode(self,):
        ...
        if novel_prob1<0.7:
            ...
            cam_mode_prob1 = np.random.rand()
            if cam_mode_prob1<0.7:
                self.render_cam_mode = 'origin'
            elif cam_mode_prob1<0.85:
        ...
        if self.render_cam_mode=='origin':
            ...
            self.render_height = self.height
            self.render_scale = 1.0
            self.render_shift_x = np.random.randn()*1
    ...
    def training_step(self, batch_input, batch_idx):
        ...
        # self.aug_novel_view_mode()  # 进行渲染的随机增广
        ...
        if batch_idx%self.save_image_duration==0:
            ...
            self._save_reprojection_images(reprojection_file, batch_render_project_data)
    ...
    @torch.no_grad()
    def predict_step(self, batch_input, recontrast_frame=[0],render_frame=[0]):
        self.stage = 'predict'
        self.init_novel_view_mode(recontrast_ids=recontrast_frame,render_ids=render_frame)
        batch_recontrast_data = self.get_recontrast_data(batch_input)
        ...
        return batch_recontrast_data, batch_render_data, batch_splating_data # batch_render_project_data, 
    ...
    def get_recontrast_data(self, inputs):
        """
        ...
        depth_maps, rot_maps, scale_maps, opacity_maps, sh_maps = self.model(inputs,self.recontrast_frame_ids)

        batch_size = depth_maps.shape[0]
        frame_camrea = self.num_cams * len(self.recontrast_frame_ids)

        c2e_extr_list = torch.stack(c2e_extr_list, dim=1)

        bfc_depth_maps = rearrange(depth_maps.squeeze(-1), 'b c h w -> (b c) h w ')
        bfc_K = inputs['K'].repeat(1, len(self.recontrast_frame_ids), 1, 1)
        bfc_K = rearrange(bfc_K, 'b c i j -> (b c) i j ')
        ...
        outputs['pts_valid'] = outputs['depth_maps'] != 0.0
        if self.stage == 'train' or self.stage == 'val':
            print('aug_mode',self.render_frame_ids,self.render_cam_mode,self.render_height,self.render_width,self.render_shift_x,self.render_shift_y)
    ...
    def get_render_data(self, inputs, render_frame_ids=None):

        zfar = self.max_depth
        znear = self.min_depth
        outputs = {}
        if render_frame_ids is None:
            render_frame_ids = self.render_frame_ids
        ...
        for frame_id in render_frame_ids:
            for cam_id in range(self.num_cams):
                ...
                for i in range(bs):
                    ...
                    projection_matrix = getProjectionMatrix(znear=znear, zfar=zfar, K=K_i, h=height, w=width).transpose(0, 1).to(K_i.device)
                ...
                outputs[('K', frame_id, cam_id)] = inputs['K'][:, cam_id, ...]
    ...
    def render_splating_imgs(self, recontrast_data, render_data, render_frame_ids=None):
        ...
        for frame_id in render_frame_ids:
            for i in range(bs):
                ...
                pts_vaild = recontrast_data['pts_valid'][i]
    ...
    def render_project_imgs(self, input_data, recontrast_data):
        outputs = {}
        ...
        for cam_id in range(self.num_cams):
            ...
            for render_frame_ids in [-1,1]:
                src_colors = input_data[('color_aug', render_frame_ids)][:, cam_id, ...]

                cam_T_cam = input_data[('cam_T_cam', 0, render_frame_ids)][:, cam_id, ...]
                ...
                outputs[('warped_gt', render_frame_ids, cam_id)] = ref_colors
                outputs[('warped_pred', render_frame_ids, cam_id)] = warped_img
                outputs[('warped_mask', render_frame_ids, cam_id)] = warped_mask.detach()
...
if __name__=='__main__':
    ...
    config_file = 'configs/nuscenes/df3dgs_inference.yaml'
    ...
    for _ in range(1):
        for key in batch_inputs:
            if isinstance(batch_inputs[key],torch.Tensor):
                batch_inputs[key] = batch_inputs[key].to('cuda:0').to(torch.float32)
        outputs = litmodel.predict_step(batch_inputs,[1])
        ...
        for i in range(1):
            ...
            projection_matrix = getProjectionMatrix(znear=1.5, zfar=100, K=K, h=height, w=width).transpose(0, 1).to('cuda:0')
