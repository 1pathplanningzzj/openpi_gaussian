...
def analyze_spatial_alignment_hypothesis(csv_path):
    ...
    if "avg_alignment" in df.columns:
        ...
        if len(success_align) > 0 and len(failure_align) > 0:
            ...
            print(
                f"  Conclusion: {'✓ Hypothesis supported - failures have significantly lower alignment' if p_val < 0.05 else '✗ No significant difference found'}"
            )
    ...
    if "num_misalignment_warnings" in df.columns:
        ...
        t_stat, p_val = stats.ttest_ind(success["num_misalignment_warnings"], failure["num_misalignment_warnings"])
        print(f"  T-test p-value: {p_val:.4f}")
        print(
            f"  Conclusion: {'✓ Failures have more warnings' if p_val < 0.05 and failure['num_misalignment_warnings'].mean() > success['num_misalignment_warnings'].mean() else '✗ No significant difference found'}"
        )
    ...
    if "avg_angle_error_deg" in df.columns:
        ...
        if len(success_angle) > 0 and len(failure_angle) > 0:
            ...
            print(f"  T-test p-value: {p_val:.4f}")
            print(
                f"  Conclusion: {'✓ Failures have larger angle error' if p_val < 0.05 and failure_angle.mean() > success_angle.mean() else '✗ No significant difference found'}"
            )
    ...
    if "target_in_view_ratio" in df.columns:
        ...
        if len(success_view) > 0 and len(failure_view) > 0:
            ...
            print(f"  T-test p-value: {p_val:.4f}")
            print(
                f"  Conclusion: {'✓ Significant difference found' if p_val < 0.05 else '✗ No significant difference found'}"
            )
    ...
    if "avg_alignment" in df.columns:
        ...
        if len(success_align) > 0 and len(failure_align) > 0:
            ...
            ax.axvline(
                ...
                linestyle="--",
                alpha=0.7,
                label=f"Success mean: {success_align.mean():.3f}",
            )
            ax.axvline(
                failure_align.mean(),
                color="red",
                linestyle="--",
                alpha=0.7,
                label=f"Failure mean: {failure_align.mean():.3f}",
            )
            ax.legend()
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default=None, help="Output directory (default: same as CSV directory)"
    )
