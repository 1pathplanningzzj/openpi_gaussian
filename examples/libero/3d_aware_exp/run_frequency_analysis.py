...
results = {"steps": [], "success_rate": [], "avg_correction": [], "max_correction": []}
...
for steps in REPLAN_STEPS_LIST:
    ...
    cmd = [
        ...
        str(NUM_TRIALS),
        "--args.video-out-path",
        f"data/analysis_steps_{steps}/videos",
    ]
    ...
    try:
        ...
        result = subprocess.run(cmd, env=env_vars, capture_output=True, text=True, check=False)
...
for i, txt in enumerate(results["success_rate"]):
    ax1.annotate(
        f"{txt:.1f}%",
        (results["steps"][i], results["success_rate"][i]),
...
ax2.plot(
    results["steps"], results["avg_correction"], "o-", color="#1f77b4", label="Avg Correction per Step", linewidth=2
)
