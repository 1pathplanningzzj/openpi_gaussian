...
with open(no_takeover_file) as f:
...
with open(with_takeover_file) as f:
    reader = csv.DictReader(f)
    for row in reader:
        if int(row["task_id"]) == 9:
            if int(row["success"]) == 1:
...
print("对比分析")
print("=" * 80)
success_rate_diff = (with_takeover_success / with_takeover_total * 100) - (
    no_takeover_success / no_takeover_total * 100
)
